  ...
  if (mode === "walk") {
    ...
    try {
      ...
      if (!res.ok || !data.features) {
        console.error("T-Map walk API failed:", res.status, JSON.stringify(data).slice(0, 300));
        throw new Error("T-Map failed");
      }
      const summary = data.features[0]?.properties;
      const walkPath = data.features
      ...
      // T-Map 실패 시 Google Routes WALK로 폴백
      const res = await fetch("https://routes.googleapis.com/directions/v2:computeRoutes", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
