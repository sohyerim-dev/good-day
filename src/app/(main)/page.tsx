  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [ownedWithInfo, collabResult] = await Promise.all([
        Promise.all(
          (owned ?? []).map(async (course) => {
            ...
            return {
              ...course,
              collabLabel: names ? `공동 편집 중 : ${names}` : "",
            };
          }),
        ),
      ...
      const collabWithInfo: CourseWithCollab[] = await Promise.all(
        ((collabResult as { data: Course[] }).data ?? []).map(
          async (course) => {
            ...
            return {
              ...course,
              collabLabel: `공동 편집 중 : ${profile?.username ?? ""}`,
            };
          },
        ),
      );
  ...
  if (!user) {
    return (
      <main className="p-4 pb-28 flex flex-col items-center relative">
        ...
        <Link
          href="/about"
          className="text-[13px] text-gray-400 border border-gray-200 rounded-2xl px-3 py-1.5 hover:border-[#EE6300] hover:text-[#EE6300] mb-4 self-start"
        >
          굿데이 소개
        </Link>
              ...
              <li key={course.id}>
                ...
                >
                  <div className="flex flex-col gap-1">
                    ...
                    {course.collabLabel && (
                      <span className="text-[11px] text-[#EE6300] pl-6">
                        {course.collabLabel}
                      </span>
                    )}
                    {course.description && (
