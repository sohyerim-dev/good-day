...
export default function EditCourse({
  ...
  const [selectedPlaces, setSelectedPlaces] = useState<(NaverPlace & { order: number })[]>([]);
  ...
  const { data: savedPlacesList = [] } = useQuery({
    queryKey: ["savedPlacesWithDetails", user?.id],
    queryFn: () => fetchSavedPlaces(user!.id),
    enabled: showSaved && !!user?.id,
  });

  // 쿼리 데이터 → 폼 상태 동기화 (최초 1회)
  useEffect(() => {
  ...
  useEffect(() => {
    if (coursePlacesData.length === 0) return;
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (placesError || !placesData) throw new Error("장소 저장 실패");

      const course_lat = placesData.reduce((sum, p) => sum + p.lat, 0) / placesData.length;
      const course_lng = placesData.reduce((sum, p) => sum + p.lng, 0) / placesData.length;

      const { error: courseError } = await supabase
        .from("courses")
        .update({ title: courseTitle, description, is_public: isPublic, course_lat, course_lng })
        .eq("id", id);

      if (courseError) throw new Error("코스 수정 실패");
  ...
  async function handleSearch() {
    ...
    const res = await fetch(`/api/search-places?query=${encodeURIComponent(query.trim())}`);
  ...
  function handleAddPlace(place: NaverPlace) {
    ...
    setQuery("");
    if (placeActive === false) setPlaceActive(true);
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setSelectedPlaces((prev) => {
      ...
      return arrayMove(prev, oldIndex, newIndex).map((place, i) => ({ ...place, order: i + 1 }));
    });
  }
  ...
  return (
    <main className="p-4 flex flex-col gap-4 pb-32">
      ...
      <label htmlFor="course-title" className="font-medium text-[18px]">코스 제목</label>
      <input
        id="course-title"
        placeholder="코스 제목"
        value={title}
      ...
      />
      <label htmlFor="course-description" className="font-medium text-[18px]">코스 설명</label>
      <textarea
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="place-search" className="font-medium text-[18px]">장소 검색</label>
      ...
      <ul className={searchActive ? "bg-gray-50 rounded-2xl p-4 w-full" : ""}>
          ...
          searchResults.map((place: NaverPlace, index: number) => (
            ...
            >
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[12px] text-gray-400">{place.roadAddress || place.address}</span>
              </div>
              <button
        ...
        <li className={searchActive ? "flex justify-end mt-1" : "hidden"}>
          <button onClick={() => setSearchActive(false)} className="text-[12px] text-gray-400 hover:text-black">
            닫기
          </button>
        </li>
      </ul>
      ...
      <div className="flex">
        <h2 className={placeActive ? "text-[18px] font-medium" : "hidden"}>나의 코스</h2>
        <Image
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={selectedPlaces.map((p) => p.id)} strategy={verticalListSortingStrategy}>
          <ul className={placeActive ? "border-2 rounded-2xl p-5 border-[#EE6300]" : ""}>
            {selectedPlaces.map((place, index) => (
              <Fragment key={place.id}>
                <SortablePlaceItem key={place.id} place={place} onRemove={() => handleRemovePlace(index)} />
                {index !== selectedPlaces.length - 1 && (
                  <li>
                    <Image src="/icons/arrow-big-down.svg" alt="" width={20} height={20} className="mx-auto mb-2" />
                  </li>
                )}
              </Fragment>
            ))}
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        ...
        <input
          ...
          className="sr-only"
        />
        <div className={`relative w-12 h-6 rounded-full transition-colors ${isPublic ? "bg-[#EE6300]" : "bg-gray-300"}`}>
        ...
        <div className="fixed inset-x-0 top-0 bottom-20 z-9999 flex flex-col justify-end">
          <div className="bg-white rounded-t-3xl p-6 shadow-lg max-h-[60vh] overflow-y-auto">
            ...
            {savedPlacesList.length === 0 ? (
              <p className="text-gray-400 text-center py-4">저장된 장소가 없어요</p>
            ) : (
              <ul className="flex flex-col gap-2">
                {savedPlacesList.map((place) => (
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      <span className="font-medium text-[14px]">{place.name}</span>
                      <span className="text-[12px] text-gray-400">{place.address}</span>
                    </div>
                    <button
                      onClick={() => { handleAddFromSaved(place); setShowSaved(false); }}
